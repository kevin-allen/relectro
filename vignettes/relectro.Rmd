...
---
This vignette documents the use of the relectro package. relectro is being developped to analyze data from electrophysiological experiments in R. The main aim of this package is to allow users to do common analysis of electrophysiological data within R. relectro relies on several C functions for computationally intensive segments of the code. This increases the speed of the analysis to a respectable level. 

You can get relectro from its github repository. From a terminal run `git clone https://github.com/kevin-allen/relectro.git`.

If you work with the source code of the package, I would recommand first reading the book "R packages" by Hadley Wickham. I am using the following packages for development: `devtools`, `roxygen2` and `testthat`.

To install relectro, you can run `R CMD build relectro` and `R CMD INSTALL relectro` from the terminal.
...
# set the intervals in spike train using output matrix from recSession in set intervals
...
df<-new("DatFiles")
df<-datFilesSet(df,
                fileNames=paste(rs@trialNames[1],"dat",sep="."),
                path=rs@path,
                nChannels=rs@nChannels)
df
data<-datFilesGetOneChannel(df,channelNo=rs@nChannels-1,
                            firstSample=rs@trialStartRes[1],
                            lastSample=rs@samplingRate*10)
length(data)
plot(data[0:100000],type='l',xlab="sampling",ylab="unscalled values",
     xlim=c(86000,89000))
...
## object to get spatial properties
sp<-new("SpatialProperties2d",session=session) 
## select position data for one environment
pt<-setInvalidOutsideInterval(pt,s=getIntervalsEnvironment(rs,env="sqr70")) 
## make firing rate maps
sp<-firingRateMap2d(sp,st,pt) 
## get info score, sparsity from maps
sp<-getMapStats(sp,st,pt) 
## spatial autocorrelation from maps
...
#firingRateMapAutosPlot(m=sp@autos,names=sp@cellList) ## for a group
...
Projects usually contain several recording sessions from different animals. ElectroProject is a class to represent all your recording sessions.
...
Now you might want to do some analysis on individual recording sessions. The trick is to create a function to do the analysis on a single session. For reasons explain below, if you function returns data, put them into a list of named elements. Then use `lapply` to run the function on individual recording sessions.
...
You can do all sorts of analysis in your function. I usually return a list of data.frames (e.g. firing rate maps, autocorrelations, etc) from the function. Because the function runs on a list of sessions, we end up with a list of lists of data.frames.

To simplify the process process of running functions on a list of session, there is a function called `runOnSessionList`. Have a look at the help to know how it works.
```{r runOnSessionList}
ep@resultsDirectory<-"/tmp"
runOnSessionList(ep,sessionList=rss,fnct=myFunction,save=T,overwrite=T,parallel=F)
```
...
If using different computers, you need to load the `relectro` package at the top of the function to make sure they all have access to relectro. You also need to make sure that the other computers have an up-to-date version of relectro. All computer should run the same version of R.

```{r parallel}
library(snow)
workers<-c(rep("localhost",4))# one computer
...
If your code is almost only reading and writing files, it is better to run the code sequencially with the lapply function. A good example of this is if you are detecting ttl pulses from the .dat files. Do this sequencially. Conversely, if your code is processor intensive and has few read/write operations, you can speed up things considerably by increasing the number of threads used.

There is nothing stopping you from running the code on several computers. Just give the ip of the computers you want to use when listing your workers. I have used up to 18 threads without any problems.
