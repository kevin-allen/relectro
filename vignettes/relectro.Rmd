...
```{r parallel}
library(snow)
#workers<-c(rep("localhost",3),rep("hm001-pc034",3),rep("hc001-pc030",3)) ## to use different computers
workers<-c(rep("localhost",4))# one computer
cl<-makeCluster(workers, type = "SOCK")
system.time(list.res<-parLapply(cl,rss,myFunction))
system.time(list.res<-lapply(rss,myFunction))
stopCluster(cl)
```
