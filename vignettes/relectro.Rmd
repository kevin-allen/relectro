...
auto<-spikeTimeAutocorrelation(st,binSizeMs=1,windowSizeMs=200,probability = F)
...
cross<-spikeTimeCrosscorrelation(st,
                                 binSizeMs=1,
                                 windowSizeMs = 200,
                                 probability = T)
...
```{r DatFiles}
df<-new("DatFiles",fileNames=paste(rs@trialNames[1],"dat",sep="."),
...
Now you might want to do some analysis on individual recording session. The trick is to create a function to do the analysis on a single session. For reasons explain below, if you function returns data, put them into a list of named elements. Then use `lapply` to run the function to individual recording session.
