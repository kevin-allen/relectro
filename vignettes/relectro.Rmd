...
Now you might want to do some analysis on individual recording session. The trick is to create a function to do the analysis on a single session. This function should return a list of named elements. Then use `lapply` to run the function to individual recording session.
...
You can do all sorts of analysis in your function. I usually return a list of data.frames (e.g. firing rate maps, autocorrelations, etc) from the function. Because the function runs on a list of sessions, we end up with a list of lists of data.frames. You can use the  `do.call("rbind")` to merge each of the matching data.frames together.

To simplify this process, there is a function called `runOnSessionList`. Have a look at the help to know how it works.
```{r runOnSessionList}
...
There is nothing stopping you from running the code on several computers. Just give the ip of the computers you want to use when listing your workers. I have used up to 20 threads without any problems.
